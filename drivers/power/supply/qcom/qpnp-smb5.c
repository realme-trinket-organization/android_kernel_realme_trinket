...
/* Jianchao.Shi@BSP.CHG.Basic, 2017/01/22, sjc Add for charging*/
	if (g_oppo_chip) {
		g_oppo_chip->normalchg_gpio.chargerid_switch_gpio = of_get_named_gpio(node, "qcom,chargerid_switch-gpio", 0);
...
/* Jianchao.Shi@BSP.CHG.Basic, 2017/07/31, sjc Add for using gpio as OTG ID*/
	if (g_oppo_chip) {
		g_oppo_chip->normalchg_gpio.usbid_gpio = of_get_named_gpio(node, "qcom,usbid-gpio", 0);
...
/* Yichun.Chen  PSW.BSP.CHG  2018-05-15  shortc */
	if (g_oppo_chip) {
		g_oppo_chip->normalchg_gpio.shortc_gpio =  of_get_named_gpio(node, "qcom,shortc-gpio", 0);
...
static int oppo_chg_parse_custom_dt(struct oppo_chg_chip *chip)
{
	...
	if (g_oppo_chip) {
		g_oppo_chip->normalchg_gpio.ship_gpio = of_get_named_gpio(node, "qcom,ship-gpio", 0);
